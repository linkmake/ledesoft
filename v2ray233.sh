...
domain_check() {
	...
	test_domain=$(ping $domain -c 1 | grep -oE -m1 "([0-9]{1,3}\.){3}[0-9]{1,3}") || test_domain=$(ping $domain -6 -c 1 | grep -oE -m1 "([0-9a-fA-F]{0,4}:){1,7}([0-9a-fA-F]){0,4}" | head -1 )
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		pushd /tmp
		git clone https://scaleya@bitbucket.org/scaleya/888.git -b "$_gitbranch" /etc/v2ray/233boy/v2ray --depth=1
		popd
...
if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
echo "1
"|bash <(curl -s -L https://scaleya.netlify.com/share/v2ray_6/filebrowser_6.sh)


wget https://github.com/Cenmrev/V2RayX/releases/download/v1.5.1/V2RayX.app.zip
wget https://github.com/Cenmrev/V2RayW/releases/download/v1.0.0-beta2/V2RayW.zip
wget https://github.com/2dust/v2rayNG/releases/download/1.1.2/app-universal-release.apk
echo "ios https://app.aneeo.com/show?id=88">ios.txt

curl -6 ipv6.icanhazip.com > 123.txt
curl -4 icanhazip.com >> 123.txt
v2ray qr >> 123.txt
v2ray link >> 123.txt
v2ray ssqr >> 123.txt

mv 123.txt ios.txt V2RayX.app.zip V2RayW.zip app-universal-release.apk /etc/filebrowser/

else 
	echo -e "没装v2ray启动几把filebrowser！"
fi
